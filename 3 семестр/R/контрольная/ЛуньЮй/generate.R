#константы которые не стоит менять
SUPPLY <- 1
SALE <- 2

#изменение расширения файлов поставок/продаж
FILE_SUPPLY <- 'in'
FILE_SALE <- 'out'
...
#список продаваемых товаров, разброс значений при генерации и цены этих товаров
GOODS <- list(
...
#по желанию директорию работы скрипта можно изменить
#setwd()
...
# далее функция обработки данных и создания файлов
#с обработанными данными и таблицами в папке Анализ 

#вспомогательная функция генерации таблицы
tab.layout <- function(){
...
#непосредственно обработка данных в таблицы в csv файлы
...
#for (i in 1){#:length(data.lst[[x]]$'in')
#задание с графиками 1
#какой магазин рассматриваем (по одному магазину надо рассмотреть график надо)
#1ый магазин х
#1ый продукт i
#Отрицательные значения будут, так как пригенерации я установил так, что
#продажи могут быть от 1 до поставки, сл-но при низких продажах рентабельность и 
#прибыль могут быть отрицательными (работает в убыток магазин)
x <- 1  
i <- 1
